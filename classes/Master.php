...
Class Master extends DBConnection {
	...
	function track_download(){
		...
		if(!isset($_SESSION['userdata']) || empty($_SESSION['userdata']['id'])){
			$resp['status'] = 'failed';
			$resp['msg'] = 'You must be logged in to download archives.';
			echo json_encode($resp);
			return;
		}
		
		// Get archive_id and validate it
		$archive_id = isset($archive_id) ? intval($archive_id) : 0;
		
		if($archive_id <= 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = 'Invalid archive ID.';
			echo json_encode($resp);
			return;
		}
		
		// Update download count
		$update = $this->conn->query("UPDATE `archive_list` SET `download_count` = `download_count` + 1 WHERE `id` = '$archive_id'");
		
		if($update) {
			$resp['status'] = 'success';
			$resp['msg'] = 'Download tracked successfully.';
			$resp['archive_id'] = $archive_id;
		} else {
			$resp['status'] = 'failed';
			$resp['msg'] = 'Failed to track download: ' . $this->conn->error;
		}
