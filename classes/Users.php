...
Class Users extends DBConnection {
	...
	public function save_student(){
		...
		if(isset($_FILES['id_front']) && $_FILES['id_front']['tmp_name'] != ''){
			$upload_dir = base_app.'uploads/idpics/';
			if(!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);
			$fname = 'uploads/idpics/front-'.$id.'.png';
			$dir_path = base_app . $fname;
			move_uploaded_file($_FILES['id_front']['tmp_name'], $dir_path);
			$this->conn->query("UPDATE student_list SET id_front = '$fname' WHERE id = '$id'");
		}
		if(isset($_FILES['id_back']) && $_FILES['id_back']['tmp_name'] != ''){
			$upload_dir = base_app.'uploads/idpics/';
			if(!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);
			$fname = 'uploads/idpics/back-'.$id.'.png';
			$dir_path = base_app . $fname;
			move_uploaded_file($_FILES['id_back']['tmp_name'], $dir_path);
			$this->conn->query("UPDATE student_list SET id_back = '$fname' WHERE id = '$id'");
		}
